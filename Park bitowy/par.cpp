...
private:
    ...
    int jumpUp(int x, int dist) {
        ...
        dist -= magic;
        magic--;
        return jumpUp(nodes[x].up[__builtin_popcount(magic)], dist);
    }
    ...
    int query(int x, int dist) {
        ...
        if (nodes[x].height >= dist) {
            return jumpUp(x, dist);
        }
        return jumpUp(nodes[x].maxDist.second, nodes[x].maxDist.first - dist);
    }
